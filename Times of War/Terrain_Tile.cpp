...
RESULT Terrain_Tile::Create(float x, float z, float scale, unsigned char subDivision)
{
	...
	lsd << "Tile Creation at (" << x << ", " << z << "), size " << scale << '\n'; 
	lsd << "Tile contains " << m_verts << " vertices.\n";
	try
	{
	...
	{
		m_factory->CreateIndexBuffer(tri_soup[h].ind.size(), USAGE_WRITEONLY, m_tris[h].ib, NULL);
...
RESULT Terrain_Tile::Render()
{
	...
	for(int i = 0; i < m_tris.size(); ++i)
	{
		tex = (ITexture*)Core::GetMediaObject(m_textureMap->GetTexture(m_tris[i].tid1));
		if(tex)
			tex->Set(0);
		tex = (ITexture*)Core::GetMediaObject(m_textureMap->GetTexture(m_tris[i].tid2));
		if(tex)
			tex->Set(1);
		m_tris[i].ib->Set();
		m_Device->SetTextureState(0, TS_COLOROP, TOP_SELECTARG1);
...
{
	Core::Function_Enter("Terrain_Tile::SetTextureMap()");
	m_textureMap = tMap;
	Core::Function_Exit();
}
