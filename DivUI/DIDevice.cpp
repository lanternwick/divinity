...
RESULT DIDevice::ProcessBuffers(char* kb_buffer, DIMOUSESTATE* mouse_data)
{
	...
	for(int i = 0; i < 256; ++i)
	{
		if(kb_buffer[i] & 0x80)
		{
			if(!keys[i])
			{
				...
				{
					if(!m_inputHandlers[j]->Keyboard_KeyDown(char(i)))
						break;
				}
			}
		...
		else
		{
			if(keys[i])
			{
				...
				for(int j = 0; j < m_inputHandlers.size(); ++j)
				{
					if(!m_inputHandlers[j]->Keyboard_KeyUp(char(i)))
						break;
				}
			}
		}
	...
	for(i = 0; i < 4; ++i)
	{
		if(mouse_data->rgbButtons[i] & 0x80)
		{
			if(!buttons[i])
			{
				...
				for(int j = 0; j < m_inputHandlers.size(); ++j)
				{
					if(!m_inputHandlers[j]->Mouse_ButtonDown(i))
						break;
				}
			}
		...
		else
		{
			if(buttons[i])
			{
				...
				for(int j = 0; j < m_inputHandlers.size(); ++j)
				{
					if(!m_inputHandlers[j]->Mouse_ButtonUp(i))
						break;
				}
			}
		}
	...
	if(mouse_data->lX || mouse_data->lY)
	{
		...
		for(int j = 0; j < m_inputHandlers.size(); ++j)
		{
			if(!m_inputHandlers[j]->Mouse_Move(mousePos, buttons[0], buttons[1], buttons[2], buttons[3]))
				break;
		}
	}
	if(mouse_data->lZ)
	{
		...
		for(int j = 0; j < m_inputHandlers.size(); ++j)
		{
			if(!m_inputHandlers[j]->Mouse_Scroll(mouseZ))
				break;
		}
	}
...
RESULT DIDevice::SetInputHandler(IInputHandler* iih)
{
	m_inputHandlers.push_back(iih);
	return OK;
}
