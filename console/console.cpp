...
void console::add_function(string& function, console_functor* f_ptr, unsigned long access_level)
{
	...
	if(s.size() == 1)
	{
		...
		fd.function_descriptor = function.c_str()+9;
...
unsigned long console::exec_command(string& cmd)
{
	std::string command(cmd);
	unsigned long i;
	for(i = 0; i < aliases.size(); ++i)
	{
		if(aliases[i].alias_name == cmd)
		{
			command = aliases[i].exec_function;
			break;
		}
	}
	string del("() ,;");
...
void console::alias(std::string& alias, std::string& function_string)
{
	unsigned long i;
	for(i = 0; i < aliases.size(); ++i)
	{
		if(aliases[i].alias_name == alias)
		{
			return;
		}
	}
	alias_def ad;
	ad.alias_name = alias;
	ad.exec_function = function_string;
...
void console::command_list(std::vector<std::string>& cmd_list)
{
	unsigned long i;
	cmd_list.clear();
	for(i = 0; i < functions.size(); ++i)
	{
		cmd_list.push_back(functions[i].function_descriptor);
	}	
	for(i = 0; i < aliases.size(); ++i)
	{
		cmd_list.push_back(aliases[i].alias_name);
	}
}